...
Item {
    ...
    Image {
        ...
        Image{
            ...
            scale: width < parent.width * 0.7 ? 1.0 : (width > 0.0 ? (parent.width / width) * 0.7 : 1.0)
           // width: 300
           // height: 500
                        ...
                        Row {
                            ...
                            Row {
                                ...
                                Image {
                                       id:               quickTipButton
                                       anchors.verticalCenter: parent.verticalCenter
                                       source:             getGift(giftType)//"qrc:/resources/images/button_quick_tip.png"
                                       width: 30
                                       height: 30
        ...
        Row {
            ...
            Image {
                ...
                MouseArea {
                    ...
                    anchors.fill: parent
                    onClicked: {
                        if (isTournament === true) return;
                        scoreListModel.clear();
                        animationTimeDown.running = true;
                        isTournament = true;
                        tournamentRatingButton.source = "qrc:/resources/images/button_tournament_rating_off.png";
                        userRatingButton.source = "qrc:/resources/images/button_user_ratings_on.png"
                        timerStartTournamentGame();
                        sendRequestGetTopToday(GenerationBranchScript.listObjectSingleLevels[currentLevel].name);
                    }
                }
            }
            ...
            Image {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        ...
                        tournamentRatingButton.source = "qrc:/resources/images/button_tournament_rating_on.png";
                        userRatingButton.source = "qrc:/resources/images/button_user_ratings_off.png"
                        userScores();
                    }
                }
            }
            ...
            Image {
                ...
                Image {
                    ...
                    MouseArea {
                        ...
                        anchors.fill: parent
                        onClicked: {
                            mainWindow.setSetting("nameUser", textInputName.text) ;
    ...
    function sendRequestGetTopToday(rating_type) {
        ...
        xhr.open('GET', "https://sakuramobile.sourceforge.io/cgi-bin/manager.cgi?key="+GenerationBranchScript.SERVER_KEY+"&action=rating_get_top_today&rating_type=" + encodeURIComponent(rating_type), true);
        xhr.send();
    ...
    function tournamentScores() {
        ...
        for (var i = 0; i < listTopUsers.length; i++) {
            var obj = GenerationBranchScript.getObjectGift(GenerationBranchScript.listObjectSingleLevels[currentLevel].name,i+1);
            if (listTopUsers[i].user_id === mainWindow.getSetting("userUuid", ""))  {
                scoreListModel.append({"name": listTopUsers[i].user_name, "score": Number(listTopUsers[i].score), "date": Qt.formatDate(new Date(), "dd.MM.yyyy"), "giftType" : obj.typeGift, "countGift" : obj.countGift,"position" : i + 1, "colorName" : "red"});
            } else {
                scoreListModel.append({"name": listTopUsers[i].user_name, "score": Number(listTopUsers[i].score), "date": Qt.formatDate(new Date(), "dd.MM.yyyy"), "giftType" : obj.typeGift, "countGift" : obj.countGift,"position" : i + 1, "colorName" : "black"});
            }
        }
    }
    ...
    function userScores() {
        ...
        db.transaction(
                    function(tx) {
                        ...
                        var res = tx.executeSql("SELECT NAME, SCORE, DATE, DIFFICULTY FROM HIGHSCORES WHERE DIFFICULTY=\"" + GenerationBranchScript.listObjectSingleLevels[currentLevel].name + "\" ORDER BY SCORE DESC LIMIT 25");
                        ...
                        for (var i = 0; i < res.rows.length; i++) {
                            if (res.rows.item(i).NAME === 'NONAME' && nameUser !== 'NONAME') {
                                name = nameUser;
                            } else {
                                name = res.rows.item(i).NAME;
                            }
                            minScore = res.rows.item(i).SCORE;
                            scoreListModel.append({"name": name, "score": Number(res.rows.item(i).SCORE), "date": Qt.formatDate(new Date(), "dd.MM.yyyy"), "position" : i + 1,"colorName" : "black","giftType" : 1, "countGift" : 1});
                        }
                        tx.executeSql("DELETE FROM HIGHSCORES WHERE SCORE < " + minScore +" AND DIFFICULTY=\"" + GenerationBranchScript.listObjectSingleLevels[currentLevel].name + "\"");
                    }
