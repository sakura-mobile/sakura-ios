...
Item {
        ...
        Rectangle {
            ...
            Image {
                ...
                MouseArea {
                    ...
                    anchors.fill: parent
                    onClicked: {                                               
                        var component = Qt.createComponent("RatingsListPage.qml");

                        if (component.status === Component.Ready) {
                            mainStackView.push(component,{currentLevel: currentLevel});
                        } else {
            ...
            Grid {
                ...
                anchors.centerIn: parent
                scale: width < parent.width * 0.9 ? 1.0 : (width > 0.0 ? (parent.width / width) * 0.9 : 1.0)
                spacing: 1
            }
    ...
    function sendRequestGetYesterday() {
        ...
        if (lastDayTourmanent !== 0) {
            ...
            if (now.getFullYear() === lastDay.getFullYear() && now.getMonth() === lastDay.getMonth() && now.getDate() === lastDay.getDate()) {return;}
        }
        var xhr = new XMLHttpRequest();
        var listGifts = [];
        xhr.timeout = 5000;
        xhr.open('GET', "https://sakuramobile.sourceforge.io/cgi-bin/manager.cgi?key="+GenerationBranchScript.SERVER_KEY+"&action=rating_get_yesterday&user_id="+encodeURIComponent(mainWindow.getSetting("userUuid", "")), true);
        xhr.send();
        ...
        xhr.onreadystatechange = function() {
          ...
          if (xhr.status != 200) {
              ...
              if (res["result"] === "success") {
                  ...
                  if (listRatings.length !== 0 && listRatings.length !== undefined) {
                      for(var i = 0; i < listRatings.length; i++) {
                          var obj = GenerationBranchScript.getObjectGift(listRatings[i].rating_type,listRatings[i].place);
                          if (obj !== false) {
                      ...
                      for(var type_gift in listGifts) {
                          if (Number(type_gift) === 1) {
                              ...
                              mainWindow.setSetting("countQuickTip", Number(mainWindow.getSetting("countQuickTip",0)) + Number(listGifts[type_gift]));
                          }
                          if (Number(type_gift) === 2) {
                              rowStepIce.visible = true;
                              textStepIce.text = listGifts[type_gift];
                              mainWindow.setSetting("countBlockStepLantern", Number(mainWindow.getSetting("countBlockStepLantern",0)) + Number(listGifts[type_gift]));
                          }
                          if (Number(type_gift) === 3) {
                              rowTimeIce.visible = true;
                              textTimeIce.text = listGifts[type_gift];
                              mainWindow.setSetting("countBlockTimeLantern", Number(mainWindow.getSetting("countBlockTimeLantern",0)) + Number(listGifts[type_gift]));
                          }
                      }
                      mainWindow.setSetting("lastDayTourmanent",now.getTime());
         ...
         for (var i = 0; i < heightGame; i++) {
             for (var j = 0; j < widthGame; j++) {
                 if (objMap.mapArray[i][j] === 1) {
                     ...
                     object.source = "qrc:/resources/images/branch/" + GenerationBranchScript.listGameBranch[i][j].name + "_3e.png";
                     ...
                     component = Qt.createQmlObject('import QtQuick 2.9; Rectangle {color: "transparent"; width: 30; height: 20}',gridMapLevelSingle);
