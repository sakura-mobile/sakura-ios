...
Item {
    ...
    Image {
        ...
        ParticleSystem {
          ...
          Emitter {
              ...
              lifeSpan:     5000
              size:         16
              emitRate:     campaniaPage.emitRatePetals
              maximumEmitted: campaniaPage.countPetals
        ...
        Flickable {
            ...
            PinchArea {
                ...
                Rectangle {
                    width: Math.max(gridMapCampania.width, backgroundFlickable.width) * 2 // !!!! * 2
                    height: Math.max(gridMapCampania.height, backgroundFlickable.height) * 2 // !!!! * 2
                    ...
                    onWidthChanged: {
                        ...
                        backgroundFlickable.initialResize(gridMapCampania.width, gridMapCampania.height);
                    }
        ...
        Row {
             anchors.horizontalCenter: parent.horizontalCenter
             anchors.bottom: parent.bottom
             anchors.bottomMargin: Math.max(campaniaPage.bannerViewHeight + 8, 20)
             spacing:                  15
             Image {
                    id:               backButton
                    anchors.bottom: parent.bottom
        ...
        Rectangle {
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    ...
                                    if (component.status === Component.Ready) {
                                        if (campaniaPage.countPetals === campaniaPage.countPetalsMax){
                                            mainStackView.push(component,{currentCampaign:currentCampaign, currentLocation: currentLocation,currentLevel: currentLevel,isCampaign: 1, listGameBranchObject : GenerationBranchScript.listGameBranchObject, isMaxPetals : 1});
                                        } else {
                                            mainStackView.push(component,{currentCampaign:currentCampaign, currentLocation: currentLocation,currentLevel: currentLevel,isCampaign: 1, listGameBranchObject : GenerationBranchScript.listGameBranchObject, isMaxPetals : 0});
                                        }
                                    } else {
            ...
            PropertyAnimation {
                ...
                properties: "y"
                easing.type: Easing.InQuad
                to :    imageBackgroundMainMap.height - rectCompletedGame.height -  Math.max(campaniaPage.bannerViewHeight + 8, 20)
                onStopped: {
                    if (!rowTextFailedGame.visible) StoreHelper.requestReview();
                }
            }
            PropertyAnimation {
    ...
    StackView.onStatusChanged: {
        if (StackView.status === StackView.Active && rectCompletedGame.y === imageBackgroundMainMap.height) {
            mixMap();
    ...
    Timer {
             ...
             onTriggered: campaniaPage.timerGame();
    }

    Timer {
             id:           timerBlockTimeGame
             interval: 1000; repeat: true
             onTriggered: campaniaPage.timerBlockTime();
    }
    ...
    function setBlockTime() {
        if (countBlockTimeLevel >= GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].countBlockTime) return;
    ...
    function setQuickTip() {
        ...
        if (countQuickTipLevel >= GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].countQuickTip) return;
    ...
    function setBlockStep() {
        if (GenerationBranchScript.isCompleted === 1) return;
        if (countBlockStepLevel >= GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].countBlockStep) return;
    ...
    function stopRotationBranchGame(ii,jj) {
        ...
        if (GenerationBranchScript.isCompletedGame() === true) {
            ...
            if (timerBlockTimeGame.running === true) timerBlockTimeGame.stop();
            setRatingUser();
            gridMapCampania.spacing = 0;
            startAnimationBranch();
            return;
        }
        if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].typeStep === 1 && GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].stepCurrent <= 0) {
            visibleFailedGameWindow();
            return;
        }
    ...
    function setRatingUser() {
        ...
        if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].typeStep) {
            if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].stepCurrent > GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].taskStepStar2)  {
                ratingGameStep = 3;
            } else if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].stepCurrent > GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].taskStepStar1 && GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].stepCurrent <= GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].taskStepStar2) {
                ratingGameStep = 2;
            } else {
                ratingGameStep = 1;
            }
        ...
        if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].typeTime) {
            if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].timeCurrent > GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].taskTimeStar2)  {
                ratingGameTime = 3;
            } else if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].timeCurrent > GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].taskTimeStar1 && GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].timeCurrent <= GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].taskTimeStar2) {
                ratingGameTime = 2;
            } else {
                ratingGameTime = 1;
            }
        ...
        var ratingGame;
        if (ratingGameStep === 0) ratingGame = ratingGameTime;
        if (ratingGameTime === 0) ratingGame = ratingGameStep;
        if (ratingGameStep !== 0 && ratingGameTime !== 0) ratingGame = ratingGameStep < ratingGameTime ? ratingGameStep : ratingGameTime

        if (ratingGame === 3) {
            campaniaPage.countPetals = campaniaPage.countPetalsMax;
            campaniaPage.emitRatePetals = campaniaPage.emitRatePetalsMax;
        ...
        if (arrUserRatingLevels[currentCampaign][currentLocation] === undefined || arrUserRatingLevels[currentCampaign][currentLocation][currentLevel] === undefined || arrUserRatingLevels[currentCampaign][currentLocation][currentLevel] < ratingGame) {
            if (arrUserRatingLevels[currentCampaign][currentLocation] === undefined) arrUserRatingLevels[currentCampaign][currentLocation] = [];
            arrUserRatingLevels[currentCampaign][currentLocation][currentLevel]  = ratingGame;
            mainWindow.setSetting("ratingLevelsUser", JSON.stringify(arrUserRatingLevels));
        }
        if (ratingGame > 0) setNewLevel();
    }
    ...
    function setNewLevel(){
        ...
        if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[newLevel] !== undefined) {
            ...
            if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[newLocation] != undefined && GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[newLocation].listLevels[0] !== undefined) {
                ...
                if (GenerationBranchScript.listObjectCampaigns[newCampaigns] != undefined && GenerationBranchScript.listObjectCampaigns[newCampaigns].listLocations[newLocation] != undefined && GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[newLocation].listLevels[0] !== undefined) {
    ...
    function mixMap() {
        campaniaPage.countPetals = campaniaPage.countPetalsMin;
        campaniaPage.emitRatePetals = campaniaPage.emitRatePetalsMin;
        ...
        while(stepGame < GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].step - 3 && arrBranch.length > 0) {
        ...
        if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].typeStep === 1) {
            GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].stepCurrent = GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].step + GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].taskStepStar3;
        } else {
            GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].stepCurrent = GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].step;
        }
        if (GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].typeTime === 1) {
            GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].timeCurrent = GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].time + GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].taskTimeStar3;
        } else {
            GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].timeCurrent = GenerationBranchScript.listObjectCampaigns[currentCampaign].listLocations[currentLocation].listLevels[currentLevel].time;
        }
    }

    function loadBranchOnMap() {

        gridMapCampania.spacing = 1;
        ...
        gridMapCampania.columns = 0
        gridMapCampania.columns = widthGame;
        for (var i = 0; i < heightGame; i++) {
            for (var j = 0; j < widthGame; j++) {
                if (objMap.mapArray[i][j] === 1) {
                    component = Qt.createComponent("Branch.qml");
                    object = component.createObject(gridMapCampania);
                    ...
                    component = Qt.createQmlObject('import QtQuick 2.9; Rectangle {color: "transparent"; width: 30; height: 20}',gridMapCampania);
